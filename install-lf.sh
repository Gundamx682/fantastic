...
install_dependencies() {
    ...
    # 如果关键工具都有，跳过安装
    if [ "$has_curl" = true ] && [ "$has_python3" = true ] && [ "$has_systemctl" = true ] && [ "$has_jq" = true ]; then
...
deploy_scripts() {
    ...
    local GITHUB_URL="https://raw.githubusercontent.com/Gundamx682/fantastic/main"
    ...
    download_and_convert() {
        ...
        log_info "下载 $filename (自动转换换行符)..."

        # 使用curl下载并使用sed转换CRLF为LF
        if curl -fsSL "${GITHUB_URL}/${filename}" | sed 's/\r$//' > "${dest_dir}/${filename}"; then
            log_info "✓ $filename 下载并转换成功"
            return 0
        else
    ...
    # 部署主脚本文件
    if ! download_and_convert "apk-downloader.sh" "$INSTALL_DIR"; then
        log_error "无法部署 apk-downloader.sh"
        exit 1
    fi
    chmod +x "$INSTALL_DIR/apk-downloader.sh"

    if ! download_and_convert "apk-server.py" "$INSTALL_DIR"; then
    ...
    # 部署systemd服务文件
    if ! download_and_convert "apk-downloader.service" "/etc/systemd/system"; then
        log_error "无法部署 apk-downloader.service"
        exit 1
    fi

    if ! download_and_convert "apk-server.service" "/etc/systemd/system"; then
